...
router.post("/Login", async (req, res, next) => {
  try {
    ...
    req.session.user_id = user.user_id;
    console.log("session user_id login: " + req.session.user_id);
    console.log("Full session after setting user_id:", req.session);
    console.log("Session ID:", req.session.id);
    
    // Force session save and set cookie manually
    try {
      // Manual cookie setting as fallback
      const sessionData = JSON.stringify({ user_id: user.user_id });
      ...
      console.log("Manual cookie error:", err);
    }
    
    // Debug response headers
    console.log("Response headers before send:", res.getHeaders());

    // return cookie with explicit headers
    res.status(200).send({ message: "login succeeded", success: true });
